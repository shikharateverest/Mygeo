...
(function (global) {
  ...
  function LatitudeTrail() {
    ...
    function updateLine() {
      ...
      latlng = new google.maps.LatLng(locations[pos].latitude, locations[pos].longitude);
    ...
    if (global.latitudeJsonData && global.latitudeJsonData.data && global.latitudeJsonData.data.items && global.latitudeJsonData.data.items.length > 0) {
      locations = global.latitudeJsonData.data.items;
      numLocations = locations.length;
      pos = numLocations - 1;
      running = true;
      updateLine();
    } else {
      console.log("No JSON Data available, make sure input.js is loaded and sets the window.latitudeJsonData variable");
    }
